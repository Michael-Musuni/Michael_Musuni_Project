...
import { ExpenseComponent } from 'src/app/billing/pages/expense/expense.component';
import { SnackbarService } from 'src/app/shared/snackbar.service';

@Component({
  selector: 'app-meter-readings-report',
  templateUrl: './meter-readings-report.component.html',
  styleUrls: ['./meter-readings-report.component.sass']
})
export class MeterReadingsReportComponent implements OnInit {
  ...
  onSubmit() {
    ...
    this.billing. meterReportReading(formattedFromDate,  formattedToDate).subscribe(
      (response: Blob) => {
        this.loading = false; // Stop loading
        const url = window.URL.createObjectURL(response);
        const a = document.createElement('a');
        a.href = url;
        a.download = 'meterReadingsReport.pdf';
