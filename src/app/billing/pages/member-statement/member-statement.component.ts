...
@Component({
  ...
  styleUrls: ['./member-statement.component.sass']
})
export class MemberStatementComponent implements OnInit {
  ...
  onSubmit() {
    ...
    const { fromDate, memberNo, toDate } = this.expensesForm.value;
    const formattedFromDate = this.formatDate(fromDate);
    const formattedToDate = this.formatDate(toDate);

    this.billing.memberStatement(formattedFromDate, memberNo, formattedToDate).subscribe(
      (response: Blob) => {
        this.loading = false; // Stop loading
        const url = window.URL.createObjectURL(response);
        const a = document.createElement('a');
        a.href = url;
        a.download = 'memberReport.pdf';
